 ...
 export  const deltaX = (x,y)=>{
    ...
    export const translateBounds = (bounds)=>{
        const {x:_x,y:_y,w:_w,h:_h} = bounds;
        const minY = deltaY(_x,_y);
        const maxY = deltaY(_x+_w,_y);
        const minX = deltaX(_x,_y);
        const maxX = deltaX(_x,_y+_h);
